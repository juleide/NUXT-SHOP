...
export default {
    ...
    async asyncData(context) {
        console.log(context)
        let detail = await service.post("/product/getDetails", {
                productID: context.query.productID
            })
        console.log(detail)
        return {
            productDetails: detail.data.Product[0]
        }
    },
    // 通过路由获取商品id
    created() {
    ...
    methods: {
        ...
        // 获取商品详细信息
        getDetails(val) {
            service.post("/product/getDetails", {
                productID: val
            })
            .then(res => {
        ...
        getDetailsPicture(val) {
            service.post("/product/getDetailsPicture", {
                productID: val
            })
            .then(res => {
        ...
        addShoppingCart() {
            ...
            service.post("/user/shoppingCart/addShoppingCart", {
        ...
        addCollect() {
            ...
            service.post("/user/collect/addCollect", {
                user_id: this.$store.getters.getUser.user_id,
                product_id: this.productID
            })
            .then(res => {
